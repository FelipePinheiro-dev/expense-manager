  ...
  if (active && payload && payload.length) {
    return (
      <CustomTooltipContainer>
        ...
        <Text className="desc">{`Category: ${payload[0].payload.category}`}</Text>
        <Text variant="success">{`Total: ${formatCurrency.format(payload[0].value)}`}</Text>
      </CustomTooltipContainer>
    )
  }
...
export function ExpensesInPeriodChart() {
  ...
  return (
    <CardContainer>
      <CardHeader title="Expenses in the period" subheader="Last 7 days" />
      <CardContent>
        {data && (
